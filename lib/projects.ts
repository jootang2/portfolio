...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: '강강: 강의 운영 자동화',
    agent: '강강',
    tagline: '출석·진도·SMS 발송까지 — 강의 운영 반복 업무를 한 화면에서 처리하는 웹앱',
    problem:
      '중등과학 강의를 운영하면서 실제 수업 외에 처리해야 할 일이 많다. 출석 체크, 진도표 관리, 부모님 공지 발송 — 하나하나는 단순하지만 매번 반복되는 작업이 누적되면 상당한 리소스가 된다.',
    process: [
      { label: '기능 기획 확정', description: '수강생 CRUD, 출석 관리, 진도 관리, SMS 발송 4가지 기능으로 범위 확정. 아침 브리핑 전화 등 복잡한 기능은 초기에 제외' },
      { label: '핵심 기능 개발', description: '수강생 관리(이름·연락처·학교·학년), 회차별 출석 체크·출석률 자동 계산, 커리큘럼 기반 진도 매트릭스 순서로 구현' },
      { label: 'SMS 연동', description: '솔라피 API 연동 완료. 진도 관리 페이지에서 학생·부모님 버튼 분리하여 인라인 발송 패널로 통합' },
      { label: '실제 데이터 반영', description: '오투 교재 커리큘럼(중1~중2, 총 37소단원) 직접 확인 후 반영. 내 수업(TeachingGrade) 기능 신규 추가' },
    ],
    result:
      'Phase 1~5 전부 완료. 수강생 10명 규모 기준 로컬에서 즉시 실행 가능. 솔라피 연동으로 버튼 하나로 학생·부모님에게 SMS 발송까지 처리된다. 오투 실제 커리큘럼 데이터가 반영되어 수업 준비 없이 바로 활용 가능한 상태.',
    status: 'complete',
    screenshots: ['/screenshots/Kangkang_1.png', '/screenshots/Kangkang_2.png'],
    tags: ['강의 운영', '교육 자동화', 'Next.js 15', 'SQLite·Prisma', '솔라피 SMS'],
    links: [
      { label: '강의 운영 자동화 구축기', url: 'https://www.gpters.org/nocode/post/automation-course-operation-student-fIE8YTeC7xvcoBr' },
    ],
    challenges: [
      {
        problem: '부모님 공지 자동화에 카카오톡을 쓰려 했으나 친구 API, PC 자동화, 봇(메신저봇R), 채널 챗봇 등 다수 방법을 검토한 결과 공기계+봇 계정+서버 상시 가동이 필요해 구축 복잡도가 과도하다고 판단됐다.',
        solution: '솔라피 SMS로 대체. 구축 복잡도는 낮고, 수강생 측 별도 조건(앱 설치 등) 없이 모두에게 발송 가능하며 비용도 미미한 수준이다.',
      },
      {
        problem: 'Prisma 최신 버전(7)에서 SQLite enum 지원 이슈가 발생했다.',
        solution: 'Prisma 5로 다운그레이드해서 해결. 실제 운영에 영향 없음.',
      },
    ],
  },
]
